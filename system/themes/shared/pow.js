...
function sha256(msg) {
  ...
  function s0(x) { return S(x, 7) ^ S(x, 18) ^ R(x, 3); }
  function s1(x) { return S(x, 17) ^ S(x, 19) ^ R(x, 10); }
  function add(x, y) { return ((x + y) | 0) >>> 0; }
  ...
  // Build padded message schedule (big-endian 32-bit words)
  var M = [];
  for (var i = 0; i < msg.length; i++)
    M[i >> 2] = (M[i >> 2] | 0) | ((msg.charCodeAt(i) & 0xff) << (24 - (i % 4) * 8));
  ...
  for (var i = 0; i < M.length; i += 16) {
    ...
    for (var t = 0; t < 64; t++) {
      if (t < 16) W[t] = M[i + t] | 0;
      else W[t] = add(add(add(s1(W[t - 2]), W[t - 7]), s0(W[t - 15])), W[t - 16]);
...
function countLeadingZeroBits(hex) {
  ...
  for (var i = 0; i < hex.length; i++) {
    ...
    if (nibble & 8) break;
    if (nibble & 4) { count += 1; break; }
    if (nibble & 2) { count += 2; break; }
    if (nibble & 1) { count += 3; break; }
    count += 4; break;
  }
  return count;
}
...
async function solve(challenge, difficulty) {
  ...
  while (true) {
    ...
    nonce++;
    // Yield to the event loop every ~80ms so the spinner keeps animating
...
if (wrapper) {
  ...
  window.startPow = function() {
    ...
    if (!window.crypto || !window.crypto.subtle) {
      errorEl.textContent = '{{ __ "pow_nosupport" }}';
    ...
    setTimeout(function() {
      solve(challenge, difficulty).then(function(nonce) {
        ...
        setTimeout(function() { document.getElementById('pow-form').submit(); }, 1200);
      }).catch(function(err) {
        document.getElementById('pow-verifying').className = 'pow-hidden';
