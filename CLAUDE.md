...
- **Comments**: Anonymous visitors can submit comments on public and password posts. When PoW is enabled, each comment submission requires a fresh one-time ALTCHA solution in the comment form; solved challenges are rejected on reuse. Comments default to `pending` status and require admin approval (`approved`/`rejected`). Configurable via `CommentsEnabled` global toggle in admin settings. Flat (no nested replies), plain text (HTML-escaped by template engine). `comments` table in SQLite with `post_id` index.
